...
from comet_problem.aws.clients.InstanceClient import InstanceClient
from comet_problem.aws.instance.DesignEvaluatorInstance import DesignEvaluatorInstance
from comet_problem.aws.instance.GeneticAlgorithmInstance import GeneticAlgorithmInstance
...
class InstanceManager:
    ...
    def __init__(self, user_info, resource_type='comet-evaluator'):
    ...
    async def instance_limit(self):
        if self.resource_type == 'comet-evaluator':
            return int(self.user_info.design_evaluator_instance_count)
        elif self.resource_type == 'comet-algorithm':
            return int(self.user_info.genetic_algorithm_instance_count)
        return 1
    ...
    async def lock(self):
        if self.resource_type == 'comet-evaluator':
            return self.user_info.design_evaluator_service_lock
        elif self.resource_type == 'comet-algorithm':
            return self.user_info.genetic_algorithm_service_lock
        return True

    async def lock_service(self):
        if self.resource_type == 'comet-evaluator':
            self.user_info.design_evaluator_service_lock = True
        elif self.resource_type == 'comet-algorithm':
            self.user_info.genetic_algorithm_service_lock = True
        await save_user_info_async(self.user_info)

    async def unlock_service(self):
        if self.resource_type == 'comet-evaluator':
            self.user_info.design_evaluator_service_lock = False
        elif self.resource_type == 'comet-algorithm':
            self.user_info.genetic_algorithm_service_lock = False
        await save_user_info_async(self.user_info)
